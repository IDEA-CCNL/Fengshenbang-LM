...
_TEST_FILE = "/cognitive_comp/huangyongfeng/fight_for_something/graph-soft-counter/data/obqa/statement/test.retri_statement.jsonl"
_TRAINING_FILE = "/cognitive_comp/huangyongfeng/fight_for_something/graph-soft-counter/data/obqa/statement/train.retri_statement.jsonl"
_DEV_FILE = "/cognitive_comp/huangyongfeng/fight_for_something/graph-soft-counter/data/obqa/statement/test.retri_statement.jsonl"
    ...
    def _generate_examples(self, filepath):
        ...
        with open(filepath, encoding="utf-8") as f:
            ...
            for idx, example in enumerate(examples):            
                ...
                # pdb.set_trace()
                # print(example['question']['choices'][0]['triple'])
                # print(example['question']['choices'][1]['triple'])
                # print(example['question']['choices'][2]['triple'])
                # print(example['question']['choices'][3]['triple'])
                if len(example['question']['choices'][0]['triple'])>0:
                    triple_0 = "[SEP]".join(['[SEP]'.join(x) for x in example['question']['choices'][0]['triple']])
                else:
                    triple_0 = ""
                if len(example['question']['choices'][1]['triple'])>0:
                    triple_1 = "[SEP]".join(['[SEP]'.join(x) for x in example['question']['choices'][1]['triple']])
                else:
                    triple_1 = ""
                if len(example['question']['choices'][2]['triple'])>0:
                    triple_2 = "[SEP]".join(['[SEP]'.join(x) for x in example['question']['choices'][2]['triple']])
                else:
                    triple_2 = ""
                if len(example['question']['choices'][3]['triple'])>0:
                    triple_3 = "[SEP]".join(['[SEP]'.join(x) for x in example['question']['choices'][3]['triple']])
                else:
                    triple_3 = ""
                yield idx, {
                    "id": example["id"],
                    ...
                    "triplets_a": {'individual_view_triplets': triple_0,
                                    ...
                                    'selected_individual_view_triplets': triple_0,
                                    # 'meaning_view_triplets': example["statements"][0]['meaning_view_triplets']
                                    },
                    "triplets_b": {'individual_view_triplets': triple_1,
                                    # 'latent_view_triplets': '[SEP]'.join(example["statements"][0]['latent_view_triplets'][0:2]),
                                    # 'group_view_triplets': '[SEP]'.join(example["statements"][0]['group_view_triplets']),
                                    'retri_view_triplets':  "",#example['question']['choices'][1]['retri_triplets'],
                                    'selected_individual_view_triplets': triple_1,
                                    # 'meaning_view_triplets': example["statements"][0]['meaning_view_triplets']
                                    },
                    "triplets_c": {'individual_view_triplets': triple_2,
                                    # 'latent_view_triplets': '[SEP]'.join(example["statements"][0]['latent_view_triplets'][0:2]),
                                    # 'group_view_triplets': '[SEP]'.join(example["statements"][0]['group_view_triplets']),
                                    'retri_view_triplets':  "",#example['question']['choices'][2]['retri_triplets'],
                                    'selected_individual_view_triplets': triple_2,
                                    # 'meaning_view_triplets': example["statements"][0]['meaning_view_triplets']
                                    },
                    "triplets_d": {'individual_view_triplets': triple_3,
                                    # 'latent_view_triplets': '[SEP]'.join(example["statements"][0]['latent_view_triplets'][0:2]),
                                    # 'group_view_triplets': '[SEP]'.join(example["statements"][0]['group_view_triplets']),
                                    'retri_view_triplets':  "",#example['question']['choices'][3]['retri_triplets'],
                                    'selected_individual_view_triplets': triple_3,
                                    # 'meaning_view_triplets': example["statements"][0]['meaning_view_triplets']
                                    },
