...
def _sample_negative_indices(
    ...
    mask_time_indices = (
        mask_time_indices.astype(np.bool) if mask_time_indices is not None else np.ones(
            features_shape, dtype=np.bool)
    )
...
class Wav2Vec2Attention(nn.Module):
    ...
    def forward(
        ...
        # because `attn_output` can be partitioned aross GPUs when using tensor-parallelism.
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            expand_attention_mask = attention_mask.unsqueeze(
                -1).repeat(1, 1, hidden_states.shape[2])
...
class Wav2Vec2Adapter(nn.Module):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(Wav2Vec2AdapterLayer(
            config) for _ in range(config.num_adapter_layers))
...
class Wav2Vec2Model(Wav2Vec2PreTrainedModel):
    ...
    def _mask_hidden_states(
        ...
        if self.config.mask_feature_prob > 0 and self.training:
            ...
            mask_feature_indices = mask_feature_indices[:,
                                                        None].expand(-1, sequence_length, -1)
    ...
    def forward(
        ...
        # add feature_grad_mult
        if self.config.feature_grad_mult > 0:
            extract_features = self.feature_extractor(input_values)
            if self.config.feature_grad_mult != 1.0:
                extract_features = GradMultiply.apply(extract_features, self.config.feature_grad_mult)
        else:
            with torch.no_grad():
                extract_features = self.feature_extractor(input_values)
...
class Wav2Vec2ForPreTraining(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        sampled_negative_indices (`torch.BoolTensor` of
        shape `(batch_size, sequence_length, num_negatives)`, *optional*):
        ...
        if sampled_negative_indices is not None:
            ...
            if self.config.sample_way == "data":
                ...
                # add for uniform sampling
                codevectors = self.quantizer.codevectors.squeeze(0)
                # (num_codevector_groups*num_codevectors_per_group,
                #    config.codevector_dim // num_codevector_groups)

                sampled_negative_indices = sampled_negative_indices.to(torch.long)
                # (B, T, num_negatives, num_codevector_groups)
                # (:, :, :, i) range in [i*num_codevectors_per_group, (i-1)*num_codevectors_per_group)

                negative_quantized_features = codevectors[sampled_negative_indices]
                # should be (B, T, num_negatives, num_codevector_groups, config.codevector_dim // num_codevector_groups)
                negative_quantized_features = negative_quantized_features.view(
                    batch_size, sequence_length, self.config.num_negatives, -1
                )
...
class Wav2Vec2ForCTC(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        ...
        output_hidden_size = (
            config.output_hidden_size if hasattr(
                config, "add_adapter") and config.add_adapter else config.hidden_size
        )
    ...
    def get_logits(
        ...
        logits = self.lm_head(hidden_states)  # should be T x B x N
        # Assume the 0 of the last dimension means blank
        ...
        # replace the padding token with inf
        masking_tensor = torch.ones(
            self.config.vocab_size, device=logits.device
        ) * float("-inf")
        masking_tensor[0] = 0
        padding = self._get_feature_vector_attention_mask(logits.shape[1], attention_mask, add_adapter=False)
        logits[1-padding] = masking_tensor

        # ctc_loss doesn't support fp16
        if normalize:
            logits = nn.functional.log_softmax(logits, dim=-1, dtype=torch.float32)
...
class Wav2Vec2ForSequenceClassification(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is None:
            ...
            pooled_output = hidden_states.sum(
                dim=1) / padding_mask.sum(dim=1).view(-1, 1)
...
class AMSoftmaxLoss(nn.Module):
    def __init__(self, input_dim, num_labels, scale=30.0, margin=0.4):
        ...
        self.weight = nn.Parameter(torch.randn(
            input_dim, num_labels), requires_grad=True)
...
class TDNNLayer(nn.Module):
    def __init__(self, config, layer_id=0):
        super().__init__()
        self.in_conv_dim = config.tdnn_dim[layer_id -
                                           1] if layer_id > 0 else config.tdnn_dim[layer_id]
...
class Wav2Vec2ForXVector(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        ...
        tdnn_layers = [TDNNLayer(config, i)
                       for i in range(len(config.tdnn_dim))]
    ...
    def forward(
        ...
        if not return_dict:
            output = (logits, output_embeddings) + \
                outputs[_HIDDEN_STATES_START_POSITION:]
            return ((loss,) + output) if loss is not None else output
