...
class JsonConfig(datasets.BuilderConfig):
    ...
    def pa_parse_options(self):
        ...
        # print(table_schema)
        return paj.ParseOptions(explicit_schema=table_schema, newlines_in_values=self.newlines_in_values, unexpected_field_behavior='ignore')
                ...
                try:
                    ...
                    with open(file, encoding="utf-8") as f:
                        ...
                        for line in tqdm(f.readlines()):
                            try:
                                assert len(json.loads(line))==9 and list(json.loads(line).keys())== ['text', 'tokens', 'record', 'entity', 'relation', 'event', 'spot', 'asoc', 'spot_asoc']
                            except:
                                print(len(json.loads(line)), json.loads(line).keys())
