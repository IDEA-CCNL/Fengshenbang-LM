from transformers import TrainingArguments,Trainer,BertModel,AutoTokenizer,AutoModelForSequenceClassification
import torch 
import json
...
def load_data(file_path,task_name,label2id):
    with open(file_path, 'r', encoding='utf8') as f:
        ...
        for line in tqdm(lines):
            ...
            text_id = int(data['id']) if 'id' in data.keys() else 0  #在该份数据集中只有测试集才有id字段
            texta,textb,labels='','',0
            if task_name=='afqmc':
                texta = data['sentence1']
                textb = data['sentence2']
                labels = label2id[data['label']] if 'label' in data.keys() else 0 #测试集中没有label标签，默认为0
            elif task_name=='tnews':
                texta = data['sentence']
                labels = label2id[data['label_desc']] if 'label_desc' in data.keys() else 0 #测试集中没有label标签，默认为0
            elif task_name=='iflytek':
                texta = data['sentence']
                labels = label2id[data['label_des']] if 'label_des' in data.keys() else 0 #测试集中没有label标签，默认为0
                ...
                textb = data['abst']
                labels = label2id[data['label']] if 'label' in data.keys() else 0 #测试集
            result.append({'texta':texta,'textb':textb,'labels':labels,'id':text_id})
        print(result[0])
        return result
...
def main():
    ...
    model = AutoModelForSequenceClassification.from_pretrained(task_args.pretrained_model_path, num_labels=len(label2id))
    tokenizer = AutoTokenizer.from_pretrained(task_args.pretrained_model_path)

    train_data = load_data(task_args.train_data_path,task_args.task_name,label2id)
    dev_data = load_data(task_args.dev_data_path,task_args.task_name,label2id)
    train_dataset = taskDataset(train_data,tokenizer=tokenizer,max_length=task_args.max_length)
    dev_dataset = taskDataset(dev_data,tokenizer=tokenizer,max_length=task_args.max_length)

    trainer = Trainer(
        model=model,                         # the instantiated   Transformers model to be trained
        args=training_args,                  # training arguments, defined above
        train_dataset=train_dataset,         # training dataset
        eval_dataset=dev_dataset,            # evaluation datase
