...
class ErLangShenBert(LightningModule):
    ...
    def __init__(self, args, tokenizer, **kwargs) -> None:
        ...
        print('use_moe:', args.use_moe)
        if args.use_moe:
            from fengshen.models.moe_bert.modeling_moe_bert import BertForPreTraining
    ...
    def setup(self, stage) -> None:
        if stage == 'fit':
            self.total_steps = get_total_steps(trainer, self.hparams)
            print('Total steps: {}' .format(self.total_steps))

    def configure_optimizers(self):
        return configure_optimizers(self, self.use_moe)
