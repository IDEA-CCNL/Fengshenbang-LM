...
from fengshen import get_args, get_tokenizer, print_rank_0
from fengshen.data.mmap_dataloader.blendable_dataset import BlendableDataset
from fengshen.data.mmap_dataloader.indexed_dataset import (
...
DSET_TYPE_BERT_CN = "bert_cn"
DSET_TYPE_GPT_CN = "gpt_cn"
...
def _build_train_valid_test_datasets(
    ...
    def build_dataset(index, name):
        from fengshen.data.mmap_dataloader.bert_cn_dataset import BertCNDataset
        from fengshen.data.mmap_dataloader.bert_dataset import BertDataset

        # from fengshen.data.mmap_dataloader.ict_dataset import ICTDataset
        from fengshen.data.mmap_dataloader.t5_dataset import T5Dataset
        ...
        if splits[index + 1] > splits[index]:
            ...
            if dataset_type == DSET_TYPE_T5:
                ...
                dataset_type == DSET_TYPE_BERT or dataset_type == DSET_TYPE_BERT_CN_WWM
            ):
                dataset = BertDataset(
                    indexed_dataset=indexed_dataset,
                    masked_lm_prob=masked_lm_prob,
                    ...
                    else "bert-cn-wwm",  # 增加参数区分bert和bert-cn-wwm
                    **kwargs,
                )
            elif dataset_type == DSET_TYPE_BERT_CN:
                dataset = BertCNDataset(
                    indexed_dataset=indexed_dataset,
                    masked_lm_prob=masked_lm_prob,
                    short_seq_prob=short_seq_prob,
                    binary_head=binary_head,
...
def get_indexed_dataset_(data_prefix, data_impl, skip_warmup):
    ...
    print_rank_0(f"    number of documents: {indexed_dataset.doc_idx.shape[0] - 1}")
...
def get_samples_mapping(
    ...
    indexmap_filename += f"_{short_seq_prob:0.2f}ssp"
    ...
    if torch.distributed.get_rank() == 0 and not os.path.isfile(indexmap_filename):
        ...
        from fengshen.data import helpers
