#!/bin/bash
#SBATCH --job-name=chimed-gpt-5shot # create a short name for your job
#SBATCH --nodes=1 # node count
#SBATCH --ntasks-per-node=1 # total number of tasks across all nodes
#SBATCH --cpus-per-task=10 # cpu-cores per task (>1 if multi-threaded tasks)
#SBATCH --mem-per-cpu=10G # memory per cpu-core (4G is default)
#SBATCH --gres=gpu:hgx:1 # number of gpus per node
#SBATCH -p pol # number of gpus per node preempted
...
MODEL_PATH=/cognitive_comp/ganruyi/Fengshenbang-LM/fengshen/workspace/ziya_llama2_13b_medical/output_v2/checkpoint-90000
...
TASK="chimed"
SAVE_PATH=./${TASK}_${MODEL_NAME}_${prefix}_predict_5shot.json
echo $SAVE_PATH
