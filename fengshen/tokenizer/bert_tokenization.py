...
class FullTokenizer:
    ...
    def __init__(self, vocab_file, do_lower_case=True, do_chinese_wwm=False):
        print(f"full vocab:{vocab_file}")
        self.vocab = load_vocab(vocab_file)
        self.inv_vocab = {v: k for k, v in self.vocab.items()}
        if do_chinese_wwm:
            self.basic_tokenizer = BasicTokenizerWithChinese(
                do_lower_case=do_lower_case
            )
            self.wordpiece_tokenizer = ChineseWordpieceTokenizer(vocab=self.vocab)
        else:
    ...
    def convert_tokens_to_string(tokens, clean_up_tokenization_spaces=True):
        ...
        def clean_up_tokenization(out_string):
            """Clean up a list of simple English tokenization artifacts like spaces before punctuations and
            abreviated forms."""
            out_string = (
                out_string.replace(" .", ".")
                .replace(" ?", "?")
...
class BasicTokenizerWithChinese(BasicTokenizer):
    """Runs basic tokenization (punctuation splitting, lower casing, etc.).

    和BasicTokenizer不同，对中英混杂做了特殊处理，见_tokenize_chinese_chars
    """
    ...
    def __init__(self, do_lower_case=True):
        ...
        try:
            ...
            self.pre_tokenizer = LAC(mode="seg")
            #  self.pre_tokenizer = lambda x: jieba.lcut(x, HMM=False)
        except ImportError:
            raise (ImportError("Chinese WWM need LAC"))
        self.do_lower_case = do_lower_case

    def _tokenize_chinese_chars(self, text):
        """Adds whitespace around any CJK character.

        如果全是中文则采用LAC分词，否则保留两边加空格的操作
        """
        output = []
        piece = ""
        for char in text:
            ...
            if self._is_chinese_char(cp):
                ...
                chinese_words = self.pre_tokenizer.run(piece)
