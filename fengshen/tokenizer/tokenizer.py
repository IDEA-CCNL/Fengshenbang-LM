...
from .bert_tokenization import convert_to_unicode
...
def build_tokenizer(args):
    ...
    elif args.tokenizer_type == "BertCNWWMTokenizer":
        tokenizer = _BertCNWWMTokenizer(
            vocab_file=args.vocab_file,
            lower_case=False,
            vocab_extra_ids=args.vocab_extra_ids,
        )
    elif args.tokenizer_type == "BertCNTokenizer":
        tokenizer = _BertWordPieceTokenizer(
            vocab_file=args.vocab_file,
            lower_case=True,
            vocab_extra_ids=args.vocab_extra_ids,
    ...
    elif args.tokenizer_type == "GPT2CNTokenizer":
        tokenizer = _GPT2WordPieceTokenizer(vocab_file=args.vocab_file)
    else:
...
class _BertWordPieceTokenizer(AbstractTokenizer):
    ...
    def tokenize(self, text):
        ...
        # print("text_tokens:{}".format(' '.join(text_tokens)))
...
def get_new_segment(segment):  # 新增的方法 ####
    ...
    :return:
    一句处理过的话 e.g. ['悬', '##灸', '技', '术', '培', '训', '专', '##家', '教', '你', '艾', '##灸', '降', '##血', '##糖']
    """
    # seq_cws = jieba.lcut("".join(segment) if isinstance(segment, list) else segment)  # 分词
    seq_cws = jieba.cut(
    ...
    while i < len(segment):  # 从句子的第一个字开始处理，知道处理完整个句子
        if (
            len(re.findall("[\u4E00-\u9FA5]", segment[i])) == 0
...
class _GPT2WordPieceTokenizer(AbstractTokenizer):
    """按照字词级别生成的tokenizer."""

    def __init__(self, vocab_file):
        name = "GPT2 WordPiece Tokenizer"
        super().__init__(name)
        self.tokenizer = FullBertTokenizer(vocab_file)
        self.eod_id = self.tokenizer.convert_tokens_to_ids(["[SEP]"])[0]  # [SEP]作为句子结尾
    ...
    def tokenize(self, text):
        ...
        text_ids = [
            id if id < self.vocab_size else id - self.vocab_size for id in text_ids
        ]  # gpt不用全词mask
        return text_ids
