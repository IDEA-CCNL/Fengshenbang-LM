...
def load_data(file_path):
    with open(file_path, 'r', encoding='utf8') as f:
        ...
        for line in tqdm(lines):
            ...
            labels = int(data['label']) if 'label' in data.keys() else 0  #测试集中没有label标签，默认为0
            text_id = int(data['id']) if 'id' in data.keys() else 0  #在该份数据集中只有测试集才有id字段
            result.append({'texta':data['sentence1'],'textb':data['sentence2'],'labels':labels,'id':text_id})
        return result
...
class taskModel(nn.Module):
    def __init__(self, pre_train_dir: str, dropout_rate: float):
        super().__init__()
        self.config = model_config[args.model_type].from_pretrained(pre_train_dir)
        self.bert_encoder = model_type[args.model_type].from_pretrained(pre_train_dir)
        self.cls_layer = torch.nn.Linear(in_features=self.config.hidden_size, out_features=1)
...
def train():
    model = taskModel(pre_train_dir=args.pretrained_model_path,dropout_rate=0).to(device)
    model.train()
    ...
    optimizer = torch.optim.AdamW(params=optimizer_grouped_parameters, lr=args.learning_rate)
    ...
    for epoch in range(args.epoch):
        ...
        for item in train_loader:
            ...
            if step%100 == 0:
                ...
                dev_loss_step,dev_acc_step=eval_dev(model,eval_func,dev_loader)
    ...
    with torch.no_grad():
        ...
        while start < len(input_text_list):
            ...
            for sent in sentence:
                input_ids0, input_seg0, input_mask0 = encoder(sent['texta'],sent['textb'])
            ...
            logit=logit.view(-1,).to(torch.device('cpu')).detach().numpy().tolist()
            logits.extend(logit)
    return logits
...
def predict():
    ...
    task_model = taskModel(pre_train_dir=args.pretrained_model_path, dropout_rate=0).to(device)
